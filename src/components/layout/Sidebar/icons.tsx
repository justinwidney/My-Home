...
export type SVGIconProps = React.SVGAttributes<SVGSVGElement> & {
	/** Pixel size or any CSS size (e.g. 20, "1.25rem") */
	size?: number | string;
	/** Defaults to currentColor so you can control via Tailwind text-* */
	stroke?: string;
	fill?: string;
	strokeWidth?: number;
	/** Override if your path isn’t in a square 20x20 or 28x28 box */
	viewBox?: string;
};
...
export const SVGIcon = React.forwardRef<SVGSVGElement, SVGIconProps>(
	(
		...
		const intrinsic = 20;
		const defaultViewBox = `0 0 ${intrinsic} ${intrinsic}`;
		...
		return (
			<svg
				...
				aria-hidden="true" /* set a title/role if you need it announced */
				{...rest}
			>
				{children}
			</svg>
...
export const Icons = {
	...
	// Add more custom icons here if you want, e.g. Wordmark, LogoMark, etc.
	// Wordmark: (props: SVGIconProps) => ( ... ),
} as const;
